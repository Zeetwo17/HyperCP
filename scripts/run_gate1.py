...
from hypercp.eval.metrics import picp, pinaw, ece, picp_hyperedge_simultaneous
from hypercp.eval.block_bootstrap import bootstrap_picp_ci, bootstrap_pinaw_ci
...
def eval_aci_hyperedge(
    ...
    for t in range(T_test):
        ...
        new_alpha = aci.state.alpha_t + aci.cfg.eta * (aci.cfg.alpha_target - err)
    ...
    # Hyperedge simultaneous coverage (Lemma 1 contract diagnostic).
    inc = incidence.numpy() if isinstance(incidence, torch.Tensor) \
        else np.asarray(incidence)
    res["picp_he_simul"] = picp_hyperedge_simultaneous(
        y_test, lowers, uppers, inc,
    )
    # Block-bootstrap CIs (Reviewer #2: time-series-aware uncertainty).
    H = y_test.shape[-1] if y_test.ndim >= 3 else 4
    res["bootstrap_picp"] = bootstrap_picp_ci(
        y_test, lowers, uppers, block_size=H,
    )
    res["bootstrap_pinaw"] = bootstrap_pinaw_ci(
        y_test, lowers, uppers, block_size=H,
    )
    return res
