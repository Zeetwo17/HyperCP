...
def picp_hyperedge_simultaneous(
    y_true: np.ndarray,
    lower: np.ndarray,
    upper: np.ndarray,
    ...
    """Hyperedge-level simultaneous coverage probability.

    For each (time step, hyperedge), checks whether ALL member nodes
    are covered by their respective prediction intervals. Returns
    the fraction of (time, hyperedge) pairs where this holds.

    This directly tests the Lemma 1 max-aggregation contract:
    if the hyperedge-level score s_e <= threshold, then ALL member
    nodes are simultaneously covered.

    Parameters
    ----------
    y_true : array of shape (T, n_firms, H)
    lower  : array of shape (T, n_firms, H)
    upper  : array of shape (T, n_firms, H)
    incidence : array of shape (n_firms, n_hyperedges)
        Binary incidence matrix. incidence[c, e] = 1 if firm c is in hyperedge e.
    mask : optional array of shape (T, n_firms, H)
        Boolean mask; True = valid observation.

    Returns
    -------
    float : fraction of (time, hyperedge) pairs where all members are covered.
    """
    y_true = _to_numpy(y_true)
    lower = _to_numpy(lower)
    upper = _to_numpy(upper)
    incidence = _to_numpy(incidence)

    node_covered = (y_true >= lower) & (y_true <= upper)
    if mask is not None:
        mask = _to_numpy(mask).astype(bool)
        node_covered = node_covered | (~mask)
    ...
    for e in range(n_hyperedges):
        ...
        for t in range(T):
            all_covered_e_t = bool(np.all(node_covered[t, members, :]))
            all_covered_vals.append(all_covered_e_t)

    if not all_covered_vals:
        return float('nan')

    return float(np.mean(all_covered_vals))
